...
describe('types', function() {
	...
	it('string value', function() {
		check('320', '320', '3200');
		check('3200', '320', '3200');
		check({ id: 'test'}, { id: 'test' });

		assert.throws(() => check({id: 123}, 'test'), CheckError);
		assert.throws(() => check({test: 123}, 'test'), CheckError);
		assert.throws(() => check(undefined, 'test'), CheckError);
		assert.throws(() => check(null, 'test'), CheckError);
		assert.throws(() => check('another string', 'test'), CheckError);
		assert.throws(() => check(/$/i, 'test'), CheckError);
		assert.throws(() => check(0, 'test'), CheckError);
		assert.throws(() => check(1, 'test'), CheckError);
		assert.throws(() => check(function() {}, 'test'), CheckError);
		assert.throws(() => check('fail', 'test'), CheckError);
		assert.throws(() => check(false, 'test'), CheckError);
		assert.throws(() => check(true, 'test'), CheckError);
	});
	...
	it('number value', function() {
		...
		assert.throws(() => check({id: 123}, 320), CheckError);
		assert.throws(() => check({test: 123}, 320), CheckError);
		assert.throws(() => check(undefined, 320), CheckError);
		assert.throws(() => check(null, 320), CheckError);
		assert.throws(() => check('another string', 320), CheckError);
		assert.throws(() => check(/$/i, 320), CheckError);
		assert.throws(() => check(0, 320), CheckError);
		assert.throws(() => check(1, 320), CheckError);
		assert.throws(() => check(function() {}, 320), CheckError);
		assert.throws(() => check('fail', 320), CheckError);
		assert.throws(() => check(false, 320), CheckError);
		assert.throws(() => check(true, 320), CheckError);
	});
	...
	it('Class', function() {
		...
		assert.throws(() => check(undefined, Test), CheckError);
		assert.throws(() => check(null, Test), CheckError);
		assert.throws(() => check('test', Test), CheckError);
		assert.throws(() => check(/$/i, Test), CheckError);
		assert.throws(() => check(0, Test), CheckError);
		assert.throws(() => check(1, Test), CheckError);
		assert.throws(() => check('fail', Test), CheckError);
